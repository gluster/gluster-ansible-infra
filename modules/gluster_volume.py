...
def remove_bricks(name, removed_bricks):
    args = ['volume', 'remove-brick', name]
    args.extend(removed_bricks)
    args_c = args[:]
    args.append('start')
    run_gluster(args)
    # remove-brick operation needs to be folowed by commit operation.
    br = removed_bricks[0]
...
def main():
    ...
    if action == 'present':
        ...
        if volume_name in volumes:
            ...
            if removed_bricks:
                remove_bricks(volume_name, removed_bricks)
                changed = True
